...
def test_define_msmnt(mocker, mocked_ff):
    mocker.patch('skrf.vi.vna.keysight.FieldFox.ntraces',
                 return_value=1, new_callable=mocker.PropertyMock)
...
def test_get_snp_network(mocker, mocked_ff):
    ...
    mocker.patch(
        'skrf.vi.vna.keysight.FieldFox.active_trace_sdata',
        return_value=mock_s_data.copy(),
