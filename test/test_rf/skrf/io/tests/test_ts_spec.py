...
def test_ex_2_write():
    ...
    stream = io.StringIO(ts.write_touchstone(
        return_string=True, form="ma", parameter="Z"))
...
ex_5_6 = rf.Network(s=np.tile(s, [2, 1, 1]), z0=[
                    50, 75, 0.01, 0.01], f=[5e9, 6e9], f_unit="Hz")
...
def test_example_8():
    ...
    ref = rf.Network(
        f=[2e6], s=[[[0.894 * np.exp(1j * -12.136 * np.pi / 180)]]])
...
def test_ex_12_g_write():
    ...
    stream = io.StringIO(ts.write_touchstone(
        return_string=True, form="ma", parameter="G"))
...
def test_ex_12_h_write():
    ...
    stream = io.StringIO(ts.write_touchstone(
        return_string=True, form="ma", parameter="H"))
...
def test_ts_example_14():
    ...
    ref = rf.Network(s=s_mag * np.exp(1j * deg / 180 * np.pi),
                     f=[5, 6, 7], f_unit="ghz")
    assert snp == ref
