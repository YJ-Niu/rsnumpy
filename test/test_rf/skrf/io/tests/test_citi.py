...
class CitiTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.twoport_example3 = Citi(
            self.test_dir + 'test_2p_citi_2params.cti')
        self.twoport_example4 = Citi(
            self.test_dir + 'test_2p_citi_2params_db.cti')
    ...
    def test_to_networkset(self):
        ...
        for example in self.examples:
            self.assertIsInstance(example.to_networkset(),
                                  rf.networkSet.NetworkSet)
    ...
    def test_values_1p_2(self):
        ...
        np.testing.assert_array_equal(
            ns_1p_2[0].f, np.array([1., 2., 3., 4., 5.]))
        np.testing.assert_array_almost_equal(ns_1p_2[0].s.squeeze(), s1p_2)

    def test_values_2p_1(self):
        """ Test if the values are correctly parsed from the CITI files """
        mag = np.array(
            [[[0.999999951, 0.000312274295], [0.000312274295, 0.999999951]]])
        deg = np.array(
            [[[-0.0178919994, 89.982108], [89.982108, -0.0178919994]]])
        ...
        np.testing.assert_array_almost_equal(
            ns_2p_1[0].s_mag[0], mag.squeeze())
        np.testing.assert_array_almost_equal(
            ns_2p_1[0].s_deg[0], deg.squeeze())
