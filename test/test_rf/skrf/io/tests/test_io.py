...
class IOTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.hfss_oneport_file = os.path.join(
            self.test_dir, 'hfss_oneport.s1p')
        self.hfss_twoport_file = os.path.join(
            self.test_dir, 'hfss_twoport.s2p')
    ...
    def read_write(self, obj):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(rf.io.read(
                os.path.join(tempdir, 'pickle.p')), obj)
    ...
    def test_save_sesh(self):
        ...
        c = self.ntwk3
        with tempfile.TemporaryDirectory() as tempdir:
            rf.io.general.save_sesh(
                locals(), os.path.join(tempdir, 'pickle.p'))
    ...
    def test_readwrite_networkSet(self):
        ...
        """
        with tempfile.TemporaryDirectory() as tempdir:
            rf.io.write(os.path.join(tempdir, 'pickle.p'),
                        rf.networkSet.NetworkSet([self.ntwk1, self.ntwk2]))
            rf.io.read(os.path.join(tempdir, 'pickle.p'))
        # self.assertEqual(rf.io.read(self.pickle_file),
        # rf.networkSet.NetworkSet([self.ntwk1, self.ntwk2])
    ...
    def test_snp_json_roundtrip(self):
        ...
        """
        given = self.ntwk1
        actual = rf.io.general.from_json_string(
            rf.io.general.to_json_string(given))
