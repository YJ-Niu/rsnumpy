...
def _compare_dicts_allclose(first: dict, second: dict) -> None:
    ...
    for k in first.keys():
        np.testing.assert_allclose(
            first[k], second[k], err_msg=f"Values from key '{k}' not equal!")
...
class DetermineTest(unittest.TestCase):
    ...
    def test_determine_reflect_matched_thru_and_line_ideal_reflect(self):
        ...
        r = determine_reflect(thru, reflect, line,
                              reflect_approx=medium.open())
        np.testing.assert_array_almost_equal(r.s, np.ones_like(r.s))
    ...
    def test_determine_reflect_matched_thru_and_line(self):
        ...
        short.s[:, 0, 0] += rng.uniform(-.02, 0.02, freq.f.size) + \
            rng.uniform(-.02, 0.02, freq.f.size)*1j
    ...
    def test_determine_reflect_regression(self):
        # this test case fails with only regularization on the t-parameters of
        # thru ** line.inv, see gh-870
    ...
    def setUp(self):
        ...
        stands_meas = [
            terminate(
                self.X**k**self.Y,
                self.gamma_f,
                self.gamma_r) for k in stands]

        self.gamma_f_indirect, self.gamma_r_indirect = compute_switch_terms(
            stands_meas)

    def test_gamma_f(self):
        self.assertTrue(
            all(np.abs(self.gamma_f_indirect.s - self.gamma_f.s) < 1e-9))

    def test_gamma_r(self):
        self.assertTrue(
            all(np.abs(self.gamma_r_indirect.s - self.gamma_r.s) < 1e-9))
...
class CalibrationTest:
    ...
    def test_from_coefs(self):
        cal_from_coefs = self.cal.from_coefs(
            self.cal.frequency, self.cal.coefs)
...
class CalibrationInputsTest(unittest.TestCase):
    ...
    def test_ideals_frequency(self):
        ...
        self.ideals[-1] = rf.Network(s=np.zeros([NPTS + 1,
                                     2, 2]), frequency=freqs)
    ...
    def test_measured_frequency(self):
        freqs = rf.Frequency(1, 10, NPTS+1, unit="GHz")
        self.measured[-1] = rf.Network(s=np.zeros([NPTS + 1,
                                       2, 2]), frequency=freqs)
        with pytest.raises(ValueError):
            self.init()
        ...
        with self.assertRaises(ValueError):
            ...
            ideals = [
                two_port_reflect(
                    wg.short(
                        name='short'), wg.short(
                        name='short')), wg.delay_short(
                    45., 'deg', name='ew'), wg.delay_short(
                        90., 'deg', name='qw'), wg.match(
                            name='load')]
            measured = [self.measure(k) for k in ideals]
            # create another calibration instance of the same class
            cal_class = type(self.cal)
            cal = cal_class(is_reciprocal=True,
                            ideals=ideals,
                            measured=measured)
            cal.run()
...
class EightTermTest(unittest.TestCase, CalibrationTest):
    ...
    def test_renormalize2(self):
        ...
        for k in self.cal.coefs.keys():
            np.testing.assert_array_almost_equal(
                old_coefs[k], self.cal.coefs[k], err_msg=k)
    ...
    def test_coefs_12term(self):
        _compare_dicts_allclose(self.cal.coefs_12term,
                                convert_8term_2_12term(self.cal.coefs))
    ...
    def test_convert_8term_2_12term_subtests(self):
        ...
        for st in [
            (self.cal.coefs['forward switch term'],
             self.cal.coefs['reverse switch term']),
            (np.zeros(self.cal.frequency.npoints, dtype=complex),
             np.zeros(self.cal.frequency.npoints, dtype=complex)),
            (self.cal.coefs['forward switch term'], np.zeros(
                self.cal.frequency.npoints, dtype=complex)),
            (np.zeros(self.cal.frequency.npoints, dtype=complex),
             self.cal.coefs['reverse switch term']),
        ]:
...
class TRLLongThruTest(EightTermTest):
    ...
    def test_found_reflect(self):
        ...
        self.assertTrue(self.cal.ideals[1] == two_port_reflect(
...
class NISTMultilineTRLTest2(NISTMultilineTRLTest):
    ...
    def setUp(self):
        ...
        l_ = 1e-9*rng.uniform(100, 200, NPTS)
        g = np.zeros(NPTS)
        c = 1e-12*rng.uniform(100, 200, NPTS)

        rlgc = DistributedCircuit(
            frequency=wg.frequency, z0=None, R=r, L=l_, G=g, C=c)
        self.rlgc = rlgc
    ...
    def test_shift(self):
        for k in self.cal.coefs.keys():
            self.assertTrue(
                all(np.abs(self.cal.coefs[k] - self.cal_shift.coefs[k]) < 1e-9))
...
class TUGMultilineTest(EightTermTest):
    thru_length = 0
    # line lengths relative to the thru
    line_offsets = [100e-6, 200e-6, 900e-6]
    cal_kwargs = {}
    ...
    def setUp(self):
        ...
        line_lengths = [self.thru_length] + \
            [self.thru_length + d for d in self.line_offsets]
...
class TUGMultilineNoReflectTest(unittest.TestCase):
    ...
    def test_transmission_correction(self):
        ...
        np.testing.assert_allclose(
            dut_cal.s[:, 1, 0], dut.s[:, 1, 0], atol=1e-6)
        np.testing.assert_allclose(
            dut_cal.s[:, 0, 1], dut.s[:, 0, 1], atol=1e-6)
...
class TwelveTermTest(unittest.TestCase, CalibrationTest):
    ...
    def test_coefs_8term(self):
        self.assertEqual(self.cal.coefs_8term,
                         convert_12term_2_8term(self.cal.coefs))
...
class TwoPortOnePathTest(TwelveTermTest):
    ...
    def test_from_coefs(self):
        cal_from_coefs = self.cal.from_coefs(
            self.cal.frequency, self.cal.coefs)
        ntwk = self.wg.random(n_ports=self.n_ports)
        f = self.cal.apply_cal((ntwk, ntwk.flipped()))
        r = self.cal.apply_cal((ntwk.flipped(), ntwk))
        self.assertEqual(f, r.flipped())
        f = cal_from_coefs.apply_cal((ntwk, ntwk.flipped()))
        r = cal_from_coefs.apply_cal((ntwk.flipped(), ntwk))
        self.assertEqual(f, r.flipped())
...
class UnknownThruTest(EightTermTest):
    def setUp(self):
        ...
        actuals = [
            ...
            wg.impedance_mismatch(50, 45)**wg.line(20, 'deg',
                                                   name='line')**wg.impedance_mismatch(45, 50)
        ...
        for k in self.cal8.coefs.keys():
            if k in self.cal16.coefs.keys():
                if 'isolation' in k:
                    continue
                self.assertTrue(
                    all(np.abs(self.cal8.coefs[k] - self.cal16.coefs[k]) < 1e-10))
                self.assertTrue(
                    all(np.abs(self.cal8.coefs[k] - self.cal_lmr16.coefs[k]) < 1e-10))
...
class MultiportCalTest(unittest.TestCase):
    ...
    def test_cal(self):
        ...
        for p in [(0, 1), (0, 2)]:
            ...
            measured = [self.measure(rf.network.twoport_to_nport(
                k, p[0], p[1], nports)) for k in actuals]
            cal_dict[p] = {}
            cal_dict[p]['method'] = NISTMultilineTRL
            cal_dict[p]['measured'] = measured
            # Two-port switch terms are in reverse order from multi-port switch
            # terms.
        ...
        for e, c in enumerate(self.cal.coefs):
            ...
            assert np.allclose(c['source match'],
                               self.Z.s[:, nports+e, nports+e])
            assert np.allclose(
                c['reflection tracking'],
                self.Z.s[:, e, nports + e] * self.Z.s[:, nports + e, e])
            assert np.allclose(c['switch term'], self.gammas[e].s)
            assert np.allclose(
                c['k'] / self.cal.coefs[0]['k'],
                self.Z.s[:, nports + 0, 0] / self.Z.s[:, nports + e, e])
        ...
        for p in [(0, 1), (0, 2)]:
            self.assertTrue(
                max(np.abs(self.wg.gamma-self.cal.cals[p].gamma)) < 1e-3)
    ...
    def make_error_networks(self, wg, nports):
        ...
        for i in range(2*nports):
            for j in range(i+1, 2*nports):
                ...
                # No connection between VNA and DUT ports with different
                # number.
                if port_type(i) == port_type(
                        j) or port_number(i) != port_number(j):
                    self.Z.s[:, i, j] = 0
                    self.Z.s[:, j, i] = 0
    ...
    def measure(self, ntwk):
        out = self.terminate(rf.network.connect(
            self.Z, self.n_ports, ntwk, 0, num=self.n_ports))
...
class MultiportSOLTTest(MultiportCalTest):
    ...
    def test_cal(self):
        ...
        for nport in nport_list:
            ...
            # thru = wg.thru(name='thru')
            thru = wg.impedance_mismatch(
                50, 45) ** wg.line(20, 'deg') ** wg.impedance_mismatch(45, 50)
            ...
            for method in method_list:
                ...
                if method == SOLT:
                    self.cal = MultiportSOLT(
                        method, measured, ideals, isolation=measured[-1])
                else:
                    self.cal = MultiportSOLT(method,
                                             measured,
                                             ideals,
                                             isolation=measured[-1],
                                             switch_terms=self.gammas)
                ...
                for e, c in enumerate(self.cal.coefs):
                    ...
                    assert np.allclose(
                        c['reflection tracking'],
                        self.Z.s[:, e, nports + e] * self.Z.s
                        [:, nports + e, e])
                    assert np.allclose(c['switch term'], self.gammas[e].s)
                    assert np.allclose(
                        c['k'] / self.cal.coefs[0]['k'],
                        self.Z.s[:, nports + 0, 0] / self.Z.s
                        [:, nports + e, e])
