...
def test_primary_plotting(
        ntwk1_dc: rf.Network,
        primary_properties: str,
        primary_methods: str):
    method = f"plot_{primary_properties}_{primary_methods}"
    fig = getattr(ntwk1_dc, method)()


def test_generated_function_plots(
        ntwk1_dc: rf.Network,
...
def test_z_time_impulse_step_z0(ntwk1_dc: rf.Network, ):
    se_diff = rf.network.concat_ports(
        [ntwk1_dc, ntwk1_dc], port_order='second')
...
def test_x_y_labels(ntwk1_dc: rf.Network):
    ...
    np.testing.assert_string_equal(
        ax.get_xlabel(), f"Frequency ({ntwk1_dc.frequency.unit})")
    # check y label
    np.testing.assert_string_equal(
        ax.get_ylabel(), rf.Network.Y_LABEL_DICT['db'])
...
def test_generated_violin_plots(
        ntwk_set_zip: NetworkSet,
        generated_functions: str):
