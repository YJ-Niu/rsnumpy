...
class VectorFittingTestCase(unittest.TestCase):
    ...
    def test_ringslot_with_proportional(self):
        ...
        vf.vector_fit(n_poles_real=2, n_poles_cmplx=0,
    ...
    def test_ringslot_without_prop_const(self):
        ...
        vf.vector_fit(n_poles_real=4, n_poles_cmplx=0,
                      fit_proportional=False, fit_constant=False)
        self.assertLess(vf.get_rms_error(), 0.01)
    ...
    def test_190ghz_measured(self):
        # perform the fit without proportional term
        s2p_file = Path(__file__).parent.parent.parent / \
            'doc/source/examples/vectorfitting/190ghz_tx_measured.S2P'
        nw = skrf.network.Network(s2p_file)
        vf = skrf.vectorFitting.VectorFitting(nw)
        vf.vector_fit(n_poles_real=4, n_poles_cmplx=4,
                      fit_proportional=False, fit_constant=True)
        self.assertLess(vf.get_rms_error(), 0.02)

    def test_no_convergence(self):
        s2p_file = Path(__file__).parent.parent.parent / \
            'doc/source/examples/vectorfitting/190ghz_tx_measured.S2P'
        # perform a bad fit that does not converge and check if a
        # RuntimeWarning is given
        nw = skrf.network.Network(s2p_file)
        vf = skrf.vectorFitting.VectorFitting(nw)
    ...
    def test_dc_enforcement(self):
        ...
        # the dc point should always be real (it still often has a tiny
        # imaginary part)
        ...
        for i in range(nw.nports):
            for j in range(nw.nports):
                ...
                abs_real_errors[i * nw.nports +
                                j] = np.abs(np.real(vf_ij - nw.s[0, i, j]))
    ...
    def test_read_write_npz(self):
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            # create a new vector fitting instance and import (read) those
            # fitted parameters
        ...
        self.assertTrue(np.allclose(
            vf.proportional_coeff, vf2.proportional_coeff))
        self.assertTrue(np.allclose(vf.constant_coeff, vf2.constant_coeff))

    @pytest.mark.skipif("matplotlib" in sys.modules,
                        reason="Raise Error only if rsplotlib is not installed.")
    ...
    def test_passivity_enforcement(self):
        ...
        # non-passive example parameters from Gustavsen's passivity assessment
        # paper:
        vf.poles = np.array([-1, -5 + 6j])
        vf.residues = np.array(
            [[0.3, 4 + 5j], [0.1, 2 + 3j], [0.1, 2 + 3j], [0.4, 3 + 4j]])
        ...
        self.assertTrue(np.allclose(violation_bands, np.array(
            [4.2472, 16.434]) / 2 / np.pi, rtol=1e-3, atol=1e-3))
